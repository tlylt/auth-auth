...
export function authenticateWithAuthorize(roleLevel) {
    return async function (req, res, next) {
        try {
            ...
            jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, (err, user) => {
                ...
                req.user = user;
                req.accessToken = token;
                console.log(user.role, roleLevel)
                if (user.role === undefined || user.role < roleLevel) {
                    return res.status(403).json({ message: "Unauthorized" });
                }
                next();
            })
        } catch (err) {
            logger.error("Fail to authenticate and authorize in middleware");
            logger.error(err);
            return res.status(500).json({ message: "Fail to authenticate and authorize in middleware" });
        }
    }
